...
(function () {
    ...
    function generatePlugin(event) {
        ...
        var dummies = document.getElementById("number").value;
        var events = documents.getElementById("events").checked;
        zip.createWriter(new zip.BlobWriter(), function (writer) {
            ...
            writer.add("src\\" + author + "\\" + name + "\\Main.php", new zip.TextReader(mainphp.replace("{author}", author).replace("{plugin}", name)), function () { },
                function (currentIndex, totalIndex) {
                    document.getElementById("status").innerHTML = "Processing src\\" + author + "\\" + name + "\\Main.php. " + currentIndex / totalIndex * 100 + "%.";
                });

            for (i = 0; i <= tasks; i++) {
                writer.add("src\\" + author + "\\" + name + "\\tasks\\" + i + ".php", new zip.TextReader(taskphp.replace("{author}", author).replace("{plugin}", name).replace("{id}", i)), function () { },
                    function (currentIndex, totalIndex) {
                        document.getElementById("status").innerHTML = "Processing src\\" + author + "\\" + name + "\\Task" + i + ".php. " + currentIndex / totalIndex * 100 + "%.";
                    });
            }
            for (i = 0; i <= dummies; i++) {
                writer.add("src\\" + author + "\\" + name + "\\Dummy" + i + ".php", new zip.TextReader(mainphp.replace("{author}", author).replace("{plugin}", name).replace("{id}", i)),
                    function (currentIndex, totalIndex) {
                        document.getElementById("status").innerHTML = "Processing src\\" + author + "\\" + name + "\\Dummy" + i + ".php. " + currentIndex / totalIndex * 100 + "%.";
                    });
            }

            writer.add("plugin.yml", new zip.TextReader(pluginyml.replace("{author}", author).replace("{plugin}", name)), function () {
